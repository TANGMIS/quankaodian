layui.use(['jquery', 'layer' ,'element','form'], function(){ 
    ...
    $(window).resize(function(){
    	var _leftCd = $(".content-right").height();
        //$(".shiti").height(parseInt(_leftCd) - 50);
        $(".st").height(parseInt(_leftCd) -50);
        $(".tab-list").height(parseInt(_leftCd) -140);
    })
    var _leftCd = $(".content-right").height();
   // $(".shiti").height(parseInt(_leftCd) - 50);
	...
	$(document).on('click','.timu-icon-return',function(){
		$(".liebiao-timu").hide();
		$(".liebiao-detail").show();
	})
	$(".dtxq-icon").click(function(){
	...
	$(document).on('click','.ckst',function(){
		var _protype = $(this).attr('data-protype');
		var _xh = $(this).attr('data');
	
		ytExamLog(_protype,_xh);
		//window.location.href=_thomedetail+"&queid="+_xh;
	})
	function ytExamLog(_protype,_xh){
		$(".layui-tab-content").hide();
		$(".kstk-dati").show();
	    $.post(_thomedetail,{'protype':_protype,'queId':_xh},function(resp){
			console.log(resp);
	        if(resp.errno==0){
				
				$('.ksdt-timu').html(resp.data);	 
				form.render();
	            $(".st").height(parseInt(_leftCd) -50);
	        }
	    },'json');
	...
	$(".show-exam-xitong").click(function(){
	...
	//开始考试
	function ytExamInfo(_cateId){
		_examLog = '';
		_zid=_cateId;
		$('.exam-chuangkou').html('<div style="font-size:18px;line-height:30px;text-align:center;width:100%;">题目加载中...</div>');
	    $.post(_ytExam,{'cateId':_cateId},function(resp){
			console.log(resp);
				$('.exam-chuangkou').html(resp);
				form.render();
	    });
	}
	function ytExamList(_cateId){
		$('.layui-tab-content').html('');
		$.post(_ytExamList,{'cateId':_cateId},function(resp){
			if(resp.status==0){
				$('.layui-tab-content').html(resp.data);
				$(".tab-list").height(parseInt(_leftCd) -140);
			}
		},'json');
	}
	...
	//检查上一题型是否做题完毕by  data-tx
	function checkIsOrNotOk(_comp){
		var _t1 = $('.show-timu span[data-tx="'+_comp+'"]');
		var _flag = 0;
		var _typeName = '';
		$.each(_t1,function(){
			var _t11 = $(this);
			if(!_t11.hasClass('yida')){
				_flag = 1;
				_typeName = _t11.attr('data-typename');
			}
		});
		if(_flag==1){
			alert('上一题型【'+_typeName+'】未答题完毕！');
			return false;
		}else{
			return true;
		}
