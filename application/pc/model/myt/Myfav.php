...
use think\Db;
use think\Config;
Class Myfav extends ModelBasic
{
    public static function getAll($where = array()){
        
        $data=Db::name("tcollect")->order("id asc")->where("userid",$where["userid"])->paginate((int)$where['limit'])->each(function($item, $key){
    ...
    {
      $id=Db::name("tcollect")->where("id",$where["id"])->where("userid",$where["userid"])->delete();
    }
  }